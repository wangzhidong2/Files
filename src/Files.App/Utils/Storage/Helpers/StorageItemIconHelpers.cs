...
namespace Files.App.Utils.Storage
{
	public static class StorageItemIconHelpers
	{
		...
		public static async Task<StorageItemThumbnail> GetIconForItemType(uint requestedSize, IconPersistenceOptions persistenceOptions = IconPersistenceOptions.Persist, string fileExtension = null)
		{
			...
			{
				StorageFolder localFolder = await StorageFolder.GetFolderFromPathAsync(AppPlatformHelper.RoamingFolderPath);
				return await localFolder.GetThumbnailAsync(ThumbnailMode.ListView, requestedSize, ThumbnailOptions.UseCurrentScale);
			}
			else
			{
				StorageFile emptyFile = await (await StorageFolder.GetFolderFromPathAsync(AppPlatformHelper.LocalCacheFolderPath)).CreateFileAsync(string.Join(Constants.Filesystem.CachedEmptyItemName, fileExtension), CreationCollisionOption.OpenIfExists);
